...
"""
    ...
    # 3. gclient sync
    # gclient requires its current working directory to be the parent of 'src'
    ...
    if os.path.exists(gclient_file):
        ...
        if "src/enterprise_browser" not in gclient_content:
            ...
            # This is a bit tricky. gclient modify --solutions won't work easily for unmanaged.
            # Best approach is to append to the .gclient file directly.
            # A more robust solution might involve parsing and modifying the .gclient file more carefully.
...
def cmd_sync(args):
    ...
    # 1. Sync Chromium (gclient sync will update submodules and might revert some core changes)
...
def cmd_patch_apply(args):
    ...
    # Load all existing .patchinfo files
    ...
    # Track which patches we processed
    ...
    for patch_file in active_patches:
        ...
        if os.path.exists(patch_info_path):
            try:
                ...
                # Brave schema: {"schemaVersion":1,"patchChecksum":"...","appliesTo":[{"path":"...","checksum":"..."}]}
    ...
    # 3. Clean up obsolete patches (info exists but patch is gone)
    for info_file in patch_info_files:
        patch_file = info_file[:-4] # Remove 'info'
...
def cmd_patch_update(args):
    ...
    if not modified_files:
        print("No modifications found in Chromium source (excluding enterprise_browser and exclusions).")
        return
    ...
    for file_path in modified_files:
        # Naming convention: replace / with - and append .patch
        ...
        if args.dry_run:
            ...
            # Check if existing patch is different
            is_different = True
    ...
    for existing_patch in os.listdir(patches_dir):
        if existing_patch.endswith('.patch') and existing_patch not in current_patches:
            if args.dry_run:
                ...
                # Also remove associated .patchinfo
...
def cmd_run(args):
    ...
    if sys.platform == 'win32':
        ...
        # For local builds, 'chrome' usually exists in the out dir
    ...
    try:
        ...
        # Use subprocess.run without check=True to allow browser to exit normally
