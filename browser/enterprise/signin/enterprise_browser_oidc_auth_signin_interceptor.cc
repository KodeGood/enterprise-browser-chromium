...
EnterpriseBrowserOidcAuthSigninInterceptor::EnterpriseBrowserOidcAuthSigninInterceptor(
    Profile* profile,
...
void EnterpriseBrowserOidcAuthSigninInterceptor::MaybeInterceptOidcAuthentication(
  ...
  if (switch_to_entry_) {
    interception_bubble_handle_ = delegate_->ShowSigninInterceptionBubble(
        ...
        base::BindOnce(
            &EnterpriseBrowserOidcAuthSigninInterceptor::OnProfileSwitchChoice,
  ...
  interception_bubble_handle_ = delegate_->ShowOidcInterceptionDialog(
      web_contents_.get(), bubble_parameters,
      base::BindOnce(
          &EnterpriseBrowserOidcAuthSigninInterceptor::OnProfileCreationChoice,
          base::Unretained(this)),
      base::BindOnce(
          &EnterpriseBrowserOidcAuthSigninInterceptor::FinalizeSigninInterception,
          base::Unretained(this)),
      base::BindRepeating(
          &EnterpriseBrowserOidcAuthSigninInterceptor::StartOidcRegistration,
          base::Unretained(this)));
}

void EnterpriseBrowserOidcAuthSigninInterceptor::Shutdown() {
  Reset();
}

void EnterpriseBrowserOidcAuthSigninInterceptor::Reset() {
  if (oidc_callback_) {
...
void EnterpriseBrowserOidcAuthSigninInterceptor::HandleError(
...
void EnterpriseBrowserOidcAuthSigninInterceptor::StartOidcRegistration() {
  ...
  auto registration_callback =
      base::BindOnce(&EnterpriseBrowserOidcAuthSigninInterceptor::OnClientRegistered,
  ...
  registration_helper_for_temporary_client_->StartRegistrationWithOidcTokens(
      oidc_tokens_.auth_token, oidc_tokens_.id_token,
      policy::BrowserDMTokenStorage::Get()->RetrieveClientId(),
      oidc_tokens_.state, timeout_duration, oidc_tokens_.is_token_encrypted,
      std::move(registration_callback));
}

void EnterpriseBrowserOidcAuthSigninInterceptor::OnClientRegistered(
  ...
  // Store the DM token in the browser-level storage to enable cloud reporting
  // and other CBCM features.
  policy::BrowserDMTokenStorage::Get()->StoreDMToken(
    dm_token_, base::BindOnce([](bool success) {
      VLOG_POLICY(1, OIDC_ENROLLMENT)
          << "Stored OIDC DM token in BrowserDMTokenStorage: " << success;
      LOG(ERROR) << "FOOBAR   DM token storage succeess: " << success;
      if (success) {
        auto* cbcm_controller = g_browser_process->browser_policy_connector()
                                    ->chrome_browser_cloud_management_controller();
                  if (cbcm_controller && cbcm_controller->report_scheduler()) {
                    cbcm_controller->report_scheduler()->OnDMTokenUpdated();
                    // Trigger an immediate report upload.
                    cbcm_controller->report_scheduler()->UploadFullReport(
                        base::DoNothing());
                  }      }
    }));
  ...
  profile_creator_ = std::make_unique<ManagedProfileCreator>(
      ...
      base::BindOnce(
          &EnterpriseBrowserOidcAuthSigninInterceptor::OnNewSignedInProfileCreated,
          base::Unretained(this)),
      preset_profile_guid);
}

void EnterpriseBrowserOidcAuthSigninInterceptor::OnProfileCreationChoice(
...
void EnterpriseBrowserOidcAuthSigninInterceptor::OnProfileSwitchChoice(
...
void EnterpriseBrowserOidcAuthSigninInterceptor::OnNewSignedInProfileCreated(
  ...
  oidc_signin_service->FetchPolicyForOidcUser(
      ...
      base::BindOnce(&EnterpriseBrowserOidcAuthSigninInterceptor::
...
void EnterpriseBrowserOidcAuthSigninInterceptor::OnPolicyFetchCompleteInNewProfile(
    bool success) {
  VLOG_POLICY(2, OIDC_ENROLLMENT)
      << "Policy fetch process for OIDC-managed profile completed with "
      << (success ? "success" : "failure");
}

void EnterpriseBrowserOidcAuthSigninInterceptor::FinalizeSigninInterception() {
  if (new_profile_) {
    VLOG_POLICY(2, OIDC_ENROLLMENT) << "Finalizing OIDC signin interception.";
    // In the Enterprise Browser, the interception might happen within the
    // Profile Picker (First Run Experience). If so, we want the FRE to finish
    // naturally for the NEW profile, instead of opening two windows.
    if (ProfilePicker::IsFirstRunOpen()) {
      // 1. Open the browser for the NEW profile FIRST.
      // This ensures that a KeepAlive exists for the new profile.
      EnterpriseBrowserOidcAuthSigninInterceptorFactory::GetForProfile(
          new_profile_.get())
          ->CreateBrowserAfterSigninInterception();

      // 2. Notify the Profile Picker that the FRE has been abandoned.
      // This marks the FRE as finished (so it won't show again) but also
      // signals that NO browser window should be opened for the original profile.
      ProfilePicker::NotifyFirstRunAbandoned();

      // 3. Schedule hiding the Profile Picker.
      base::SingleThreadTaskRunner::GetCurrentDefault()->PostTask(
          FROM_HERE, base::BindOnce(&ProfilePicker::Hide));
    } else {
      // Standard flow: just open the browser.
      EnterpriseBrowserOidcAuthSigninInterceptorFactory::GetForProfile(
          new_profile_.get())
          ->CreateBrowserAfterSigninInterception();
    }
...
void EnterpriseBrowserOidcAuthSigninInterceptor::
    CreateBrowserAfterSigninInterception() {
  // Open a new browser.
