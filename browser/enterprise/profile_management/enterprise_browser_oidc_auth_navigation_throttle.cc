...
#include "enterprise_browser/browser/enterprise/profile_management/enterprise_browser_oidc_auth_navigation_throttle.h"
...
constexpr char kEnrollmentFallbackUrl[] =
    "https://console.your-enterprise-browser.com/enroll";
...
// Enterprise Browser page that handles OIDC authentication redirection, this
...
constexpr char kEnterpriseOidcRegisterUrl[] =
    "http://localhost:3001/profile-enrollment/register-handler";
...
// static
void EnterpriseBrowserOidcAuthNavigationThrottle::MaybeCreateAndAdd(
    content::NavigationThrottleRegistry& registry) {
  if (base::FeatureList::IsEnabled(
          profile_management::features::kOidcAuthProfileManagement) &&
      registry.GetNavigationHandle().IsInPrimaryMainFrame()) {
    registry.AddThrottle(
        std::make_unique<EnterpriseBrowserOidcAuthNavigationThrottle>(registry));
  }
}
...
EnterpriseBrowserOidcAuthNavigationThrottle::
    ~EnterpriseBrowserOidcAuthNavigationThrottle() = default;

ThrottleCheckResult
EnterpriseBrowserOidcAuthNavigationThrottle::WillRedirectRequest() {
  return AttemptToTriggerUrlInterception();
}

ThrottleCheckResult
EnterpriseBrowserOidcAuthNavigationThrottle::WillProcessResponse() {
...
const char* EnterpriseBrowserOidcAuthNavigationThrottle::GetNameForLogging() {
  return "EnterpriseBrowserOidcAuthNavigationThrottle";
}

// static
std::unique_ptr<URLMatcher> EnterpriseBrowserOidcAuthNavigationThrottle::
    GetOidcEnrollmentUrlMatcherForTesting() {
  return CreateOidcEnrollmentUrlMatcher();
}

ThrottleCheckResult
EnterpriseBrowserOidcAuthNavigationThrottle::AttemptToTriggerUrlInterception() {
  ...
  data_decoder::DataDecoder::ParseJsonIsolated(
      json_payload,
      base::BindOnce(
          &EnterpriseBrowserOidcAuthNavigationThrottle::RegisterWithOidcTokens,
...
void EnterpriseBrowserOidcAuthNavigationThrottle::RegisterWithOidcTokens(
  ...
  auto* interceptor = EnterpriseBrowserOidcAuthSigninInterceptorFactory::GetForProfile(
  ...
  interceptor->MaybeInterceptOidcAuthentication(
      ...
      std::string(),
      base::BindOnce(&EnterpriseBrowserOidcAuthNavigationThrottle::Resume,
                     weak_ptr_factory_.GetWeakPtr()));
}

ThrottleCheckResult EnterpriseBrowserOidcAuthNavigationThrottle::
  ...
  if (std::optional<std::string> registration_header =
    ...
    auto* interceptor =
        EnterpriseBrowserOidcAuthSigninInterceptorFactory::GetForProfile(profile);
